    ...
    with Progress() as bar:
        ...
        def configure_system_mac(task: Task, device_flash: Path) -> bool:
            ...
            if device_system_mac.exists():
                bar.console.log(f"{task.host}: System MAC address already configured. Use 'containerlab destroy --cleanup' if you want to override.")
                return False
        ...
        def configure_serial_number(task: Task, device_flash: Path) -> bool:
            ...
            if device_serial_number.exists():
                bar.console.log(f"{task.host}: Serial Number already configured. Use 'containerlab destroy --cleanup' if you want to override.")
                return False
        ...
        def copy_cv_token(task: Task, device_flash: Path) -> bool:
            ...
            if token is None:
                bar.console.log(f"{task.host}: CloudVision token has not been provided. Not configuring...")
                return False
